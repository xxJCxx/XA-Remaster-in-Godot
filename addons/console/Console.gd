...
func _ready() -> void:
	...
	rich_label.text = "Consola de desarrollo de XA.\n"
	...
	add_command("puntos", points, 1)
	add_command("vidas", lifes, 1)
	add_command("quitar", quit, 0)
	add_command("salir", quit, 0)
	add_command("limpiar", clear, 0)
	add_command("borrar_historial", delete_history, 0)
	add_command("ayuda", help, 0)
	add_command("lista_comandos", commands_list, 0)

func points(puntos):
	var player = get_tree().get_first_node_in_group("player")
	player.añadir_puntos(puntos)
func lifes(vidas):
	var player = get_tree().get_first_node_in_group("player")
	player.añadir_vidas(vidas)
	...
	if (split_text.size() > 0):
		...
		if (console_commands.has(command_string)):
			...
			match command_entry.param_count:
				...
				_:
					print_line("No se soportan comandos con mas de 3 parametros")
		else:
			emit_signal("Comando no encontrado")
			print_line("El comando no funcina")
